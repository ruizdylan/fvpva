# Create your views here.
import datetime

from django.db.models import Q
from model_utils import Choices
from rest_framework import status
from rest_framework.authentication import TokenAuthentication
from rest_framework.parsers import MultiPartParser

from api.order.models import Order
from api.permissions import IsOauthAuthenticatedSuperAdmin, IsGETorIsOauthAuthenticatedSuperAdmin
from api.services.models import Services, ServiceNumber
from api.services.serializer import ServicesDataTableSerializer, ServicesNumberSerializer, \
    ServicesNumberHomeSerializer
from api.views import BaseAPIView
from fapva.utils import query_datatable_by_args_countries, boolean


class ServicesDataTableAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            query_object = Q(is_deleted=False)
            # city = request.query_params.get('city', "")
            # type = request.query_params.get('type', "")
            query = request.query_params.get('query', '')
            #
            # premium_request = request.query_params.get('premium_request', None)
            # featured_request = request.query_params.get('featured_request', None)
            # if city:
            #     query_object = ()
            # query_object = Q()
            # activated = request.query_params.get('status', '3')
            # if query:
            #     query_object &= Q(status=int(query))
            # else:
            #     query_object &= Q(status__in=[0, 1, 2, 3])
            # elif status == '2':
            #     expired = True
            # if status == '3':
            #     query_object = Q(status__in=[1, 2])
            # elif status == '4':
            #     query_object = Q(is_expired=True, status__in=[1, 2])
            # elif activated == '2':
            #     query_object = Q(status=int(activated)) | Q(is_expired=True, status=1)
            # elif activated == '1':
            #     query_object = Q(is_expired=False, status=int(activated))

            # if type:
            #     query_object &= Q(type=type)
            # if city:
            #     query_object &= Q(city=city)
            # if is_approved:
            #     query_object &= Q(is_approved=boolean(is_approved))
            ORDER_COLUMN_CHOICES = Choices(
                ('0', 'id'),
                ('1', 'type'),
                ('4', 'id'),
                ('6', 'description'),
                ('7', 'price')
            )
            property_ = query_datatable_by_args_countries(
                kwargs=request.query_params,
                model=Services,
                query_object=query_object,
                ORDER_COLUMN_CHOICES=ORDER_COLUMN_CHOICES,
                search_function=self.search_countries
            )

            serializer = ServicesDataTableSerializer(property_.get('items', []), many=True)

            property_data = {
                'draw': property_.get('draw', 0),
                'recordsTotal': property_.get('total', 0),
                'recordsFiltered': property_.get('count', 0),
                'data': serializer.data,
            }
            description = 'List of jobs'

            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                payload=property_data,
                description=description
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description=str(e)
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )

    @staticmethod
    def search_countries(queryset, search_value, kwargs):
        """
        search given queryset with given search_value
        :param list queryset: all records of a model
        :param str search_value: value user enter in datatable search
        :param dict kwargs: request param from datatable
        :rtype:list
        """
        try:
            query_object = Q(id__icontains=search_value) | Q(name__icontains=search_value)
            query_object |= Q(code__icontains=search_value)
            return queryset.filter(query_object)
        except:
            return []


class ServicesNumberDataTableAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            query_object = Q(is_deleted=False)
            # city = request.query_params.get('city', "")
            # type = request.query_params.get('type', "")
            query = request.query_params.get('query', '')
            #
            # premium_request = request.query_params.get('premium_request', None)
            # featured_request = request.query_params.get('featured_request', None)
            # if city:
            #     query_object = ()
            # query_object = Q()
            # activated = request.query_params.get('status', '3')
            # if query:
            #     query_object &= Q(status=int(query))
            # else:
            #     query_object &= Q(status__in=[0, 1, 2, 3])
            # elif status == '2':
            #     expired = True
            # if status == '3':
            #     query_object = Q(status__in=[1, 2])
            # elif status == '4':
            #     query_object = Q(is_expired=True, status__in=[1, 2])
            # elif activated == '2':
            #     query_object = Q(status=int(activated)) | Q(is_expired=True, status=1)
            # elif activated == '1':
            #     query_object = Q(is_expired=False, status=int(activated))

            # if type:
            #     query_object &= Q(type=type)
            # if city:
            #     query_object &= Q(city=city)
            # if is_approved:
            #     query_object &= Q(is_approved=boolean(is_approved))
            ORDER_COLUMN_CHOICES = Choices(
                ('0', 'id'),
                ('1', 'type'),
                ('4', 'id'),
                ('6', 'description'),
                ('7', 'price')
            )
            property_ = query_datatable_by_args_countries(
                kwargs=request.query_params,
                model=ServiceNumber,
                query_object=query_object,
                ORDER_COLUMN_CHOICES=ORDER_COLUMN_CHOICES,
                search_function=self.search_services_number
            )

            serializer = ServicesNumberSerializer(property_.get('items', []), many=True)

            property_data = {
                'draw': property_.get('draw', 0),
                'recordsTotal': property_.get('total', 0),
                'recordsFiltered': property_.get('count', 0),
                'data': serializer.data,
            }
            description = 'List of jobs'

            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                payload=property_data,
                description=description
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description=str(e)
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )

    @staticmethod
    def search_services_number(queryset, search_value, kwargs):
        """
        search given queryset with given search_value
        :param list queryset: all records of a model
        :param str search_value: value user enter in datatable search
        :param dict kwargs: request param from datatable
        :rtype:list
        """
        try:
            query_object = Q(id__icontains=search_value) | Q(country__name__icontains=search_value)
            query_object |= Q(service__name__icontains=search_value)
            query_object |= Q(number__icontains=search_value)
            query_object |= Q(price__icontains=search_value)
            return queryset.filter(query_object)
        except:
            return []


class ServicesStatusUpdateAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            country = Services.objects.get(id=pk)
            country.is_active = not country.is_active
            country.save()
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                description="Status Changed Successfully"
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Service with this id does not exists in the system."
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )


class ServicesNumberStatusUpdateAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            country = ServiceNumber.objects.get(id=pk)
            country.is_active = not country.is_active
            country.save()
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                description="Status Changed Successfully"
            )

        except ServiceNumber.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Service with this id does not exists in the system."
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )


class ServicesAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsGETorIsOauthAuthenticatedSuperAdmin,)
    parser_classes = (MultiPartParser,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            country_id = request.query_params.get('country_id', None)
            is_paid = request.query_params.get('is_paid', None)
            query_object = Q()
            if country_id:
                query_object &= Q(country_id=country_id)
            if is_paid:
                query_object &= Q(is_paid=boolean(is_paid))

            # :todo Check if the number is completed or cancelled or is faulty

            number_list = Order.objects.filter(
                is_completed=True,
            ).values_list("number_id")
            country = ServiceNumber.objects.filter(
                query_object,
                is_active=True,
                is_deleted=False,
                service__is_active=True,
                service__is_deleted=False,
                country__is_active=True,
                country__is_deleted=False,
            ).exclude(id__in=[d[0] for d in number_list])
            serializer = ServicesNumberHomeSerializer(country, many=True)
            return self.send_response(
                success=True,
                payload=serializer.data,
                status_code=status.HTTP_200_OK,
                description="Service Data"
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description=str(e)
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )

    def post(self, request, pk=None):
        try:
            serializer = ServicesDataTableSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return self.send_response(
                    success=True,
                    code=f'200',
                    status_code=status.HTTP_200_OK,
                    payload=serializer.data,
                    description='Service Created Successfully',
                )
            else:
                return self.send_response(
                    success=False,
                    code='422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description=serializer.errors
                )
        except Exception as e:
            if hasattr(e.__cause__, 'pgcode') and e.__cause__.pgcode == '23505':
                return self.send_response(
                    code=f'422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description="Country with this name already exists"
                )
            return self.send_response(
                code=f'500',
                description=e
            )

    def put(self, request, pk=None):
        try:
            instance = Services.objects.get(id=pk)
            serializer = ServicesDataTableSerializer(instance=instance, data=request.data)
            if serializer.is_valid():
                serializer.save()
                return self.send_response(
                    success=True,
                    code=f'200',
                    status_code=status.HTTP_200_OK,
                    payload=serializer.data,
                    description='Service Updated Successfully',
                )
            else:
                return self.send_response(
                    success=False,
                    code='422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description=serializer.errors
                )
        except Exception as e:
            if hasattr(e.__cause__, 'pgcode') and e.__cause__.pgcode == '23505':
                return self.send_response(
                    code=f'422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description="Country with this name already exists"
                )
            return self.send_response(
                code=f'500',
                description=e
            )


class ServicesDeleteAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            country = Services.objects.get(id=pk)
            country.is_deleted = True
            country.save()
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                description="Deleted Successfully"
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description=str(e)
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )


class ServicesNumberDeleteAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            country = ServiceNumber.objects.get(id=pk)
            country.is_deleted = True
            country.save()
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                description="Deleted Successfully"
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description=str(e)
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )


class ServicesCountryNumberAPIView(BaseAPIView):
    """
    API View for Login Super Admin and Admin
    """
    authentication_classes = (TokenAuthentication,)
    permission_classes = (IsOauthAuthenticatedSuperAdmin,)
    parser_classes = (MultiPartParser,)

    def get(self, request, pk=None):
        """
        :param request:
        :param pk: to get singal instance of property
        :return: response of required properties listings
        """
        try:
            country = ServiceNumber.objects.get(id=pk)
            country.is_active = not country.is_active
            country.save()
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                description="Status Changed Successfully"
            )

        except Services.DoesNotExist as e:
            return self.send_response(
                code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description=str(e)
            )
        except Exception as e:
            return self.send_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                description=str(e)
            )

    def post(self, request, pk=None):
        try:
            serializer = ServicesNumberSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return self.send_response(
                    success=True,
                    code=f'200',
                    status_code=status.HTTP_200_OK,
                    payload=serializer.data,
                    description='Service Created Successfully',
                )
            else:
                return self.send_response(
                    success=False,
                    code='422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description=serializer.errors
                )
        except Exception as e:
            if hasattr(e.__cause__, 'pgcode') and e.__cause__.pgcode == '23505':
                return self.send_response(
                    code=f'422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description="Country with this name already exists"
                )
            return self.send_response(
                code=f'500',
                description=e
            )

    def put(self, request, pk=None):
        try:
            instance = ServiceNumber.objects.get(id=pk)
            serializer = ServicesNumberSerializer(instance=instance, data=request.data)
            if serializer.is_valid():
                serializer.save()
                return self.send_response(
                    success=True,
                    code=f'200',
                    status_code=status.HTTP_200_OK,
                    payload=serializer.data,
                    description='Data Updated Successfully',
                )
            else:
                return self.send_response(
                    success=False,
                    code='422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description=serializer.errors
                )
        except Exception as e:
            if hasattr(e.__cause__, 'pgcode') and e.__cause__.pgcode == '23505':
                return self.send_response(
                    code=f'422',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    description="Country with this name already exists"
                )
            return self.send_response(
                code=f'500',
                description=e
            )
